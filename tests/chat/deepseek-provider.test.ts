...
import { DeepseekProvider } from '../../src/domains/chat/infrastructure/ai-provider-adapter';
...
describe('DeepseekProvider', () => {
  ...
  it('yields content and ignores reasoning_content', async () => {
  ...
  it('sends correct request format', async () => {
    const provider = new DeepseekProvider('test-key', 'custom-model');
    const lines = ['data: [DONE]'];
    vi.mocked(global.fetch).mockResolvedValue(createMockSSEResponse(lines));
